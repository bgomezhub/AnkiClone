...
class ConjugationQuizPage:
    def __init__(self, frame, word_list):
        ...
        if self.is_new == 1:
            ...
            # Submission
            self.submit = ctk.CTkButton(self.f_submission, text="Submit", command=lambda: self.submission(word_list))
    ...
    def define_conjugation_table(self):
        ...
        if self.is_new == 1:
            ...
            QuizManager.build_table_new_word(self.f_conjugation_table, con_subjects, 14, word=self.conjugation)
        else:
            # Define conjugation question (top)
            QuizManager.quiz_title(self.f_question, self.word)
            # Return table widgets
            return QuizManager.build_table(self.f_conjugation_table, con_subjects, 14)
    ...
    def submission(self, word_list):
        ...
        grade = QuizManager.table_feedback(
            self.f_conjugation_table, self.conjugation_table, self.conjugation, 14)
